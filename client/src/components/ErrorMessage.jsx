const ErrorMessage = ({ message, arrowDir, position }) => {
  ...
  text-md lg:w-88 p-4 bg-red-800/90 mb-4`}
    >
      {message}
      <div
        className={`${arrowStyle} shadow-xl shadow-red-800/50 w-0 h-0 absolute  border-t-[14px] 
        border-t-red-800/75 border-x-[12px] border-x-transparent`}
      ></div>
    </div>
  );
};
