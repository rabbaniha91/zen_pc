...
const EditAdderss = ({ setEnterShipping, setShipping, current }) => {
  ...
  const lettetAndNumRegex =
    /^[چجحخهعغفقثصضشسیبلاتنمکگ.وئدذرزطظ1234567890 ءپ]+$/;
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      const { signal } = controller;
      setIsLoading(true);
      editAddress(axiosPrivate, address, signal, current?._id)
        .then((data) => {
          setShipping(data)
          setIsLoading(false);
